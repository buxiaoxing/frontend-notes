...
```mermaid
graph TB
A[浏览器模块化的弊端]-->B[在浏览器端开发时态和运行时态的侧重点不一样]-->C[解决办法: 需要构建工具将开发时态和运行时态分开]-->D[webpack就是这样的工具]
```
...
- 浏览器模块化的弊端
  ...
  1. 效率问题
     ...
     精细的模块划分带来了**更多的JS文件**，更多的JS文件带来了更多的请求，降低了页面访问效率

  2. 兼容性问题

     浏览器目前仅支持**ES6**的模块化标准，并且还存在兼容性问题

      npm上大部分第三方包都是基于**commonJs**模块化。

  3. 工具问题

     浏览器不支持npm下载的**第三方包**，没有默认导入路径，需要一级一级路径导入

  4. 还有非常多的非业务问题

     包括：执行效率、兼容性、代码的可维护性可扩展性、团队协作、测试等等。这些都是**工程化问题**。

  *node的文件都在本地，所以没有这些问题*

- 在浏览器端**开发时**和**运行时**的侧重点不一样
...
- 解决办法
  ...
  我们需要有一个工具，这个工具能够让开发者专心的在开发时态写代码，然后利用这个工具将开发时态编写的代码转换为运行时态需要的东西。

    ![image-20221209164219099](http://img.buxiaoxing.com/uPic/2022/12/09164220-s52McP-image-20221209164219099.png)

- webpack就是这样的工具

  ![image-20221209164411075](http://img.buxiaoxing.com/uPic/2022/12/09164412-ChB2dD-image-20221209164411075.png)
...
> webpack是**基于模块化**的打包（构建）工具，它把一切视为模块
>
...
- **基于nodejs**：由于webpack在构建的过程中需要读取文件，因此它是**运行在node环境**中的
...
由于webpack同时支持CommonJS和ES6 module，因此需要理解它们**互操作**时webpack是如何处理的

#### 同模块化标准

如果导出和导入使用的是同一种模块化标准，打包后的效果和之前学习的模块化没有任何差异

![image-20230216001917279](http://img.buxiaoxing.com/uPic/2023/02/16001917-XQDahZ-image-20230216001917279.png)

![image-20230216001939734](http://img.buxiaoxing.com/uPic/2023/02/16001939-aPQXHy-image-20230216001939734.png)

#### 不同模块化标准

不同的模块化标准，webpack按照如下的方式处理

![image-20230216002001146](http://img.buxiaoxing.com/uPic/2023/02/16002001-i9L7Nq-image-20230216002001146.png)

![image-20230216002018028](http://img.buxiaoxing.com/uPic/2023/02/16002018-yzmLhv-image-20230216002018028.png)
...
(function(modules){
  ...
  // require函数做的就是通过传入的路径，执行函数，获取module.exports或者exports的值
  function __webpack_require__(moduleId){
    ...
    __webpack_cache__[moduleId] = result
    return result
  }
...
将每个模块的路径作为key，该模块代码放在一个函数中作为value(将`module, export, require`都作为参数传入)。保证了**不污染全局变量**，也不回使用任何模块化的内容，只用js最基础的函数实现，确保了**兼容性**。

使用 `eval` 执行代码调试时能够看到源码
...
**注意：配置文件中的代码，必须是有效的node代码**(webapck运行在node环境中)
...
1. mode：编译模式，字符串，取值为`development`或`production`，指定编译结果代码运行的环境，会影响webpack对编译结果代码格式的处理
   ...
   `development`的代码的更方便调试

   `production`的代码更小更丑，效率更高
...
![image-20230216150837434](http://img.buxiaoxing.com/uPic/2023/02/16150837-pTxlDD-image-20230216150837434.png)
...
![image-20230216150927320](http://img.buxiaoxing.com/uPic/2023/02/16150927-0Thjef-image-20230216150927320.png)

![image-20230216150951145](http://img.buxiaoxing.com/uPic/2023/02/16150951-Fi8DPf-image-20230216150951145.png)
...
![image-20230216151324939](http://img.buxiaoxing.com/uPic/2023/02/16151325-AIsRU5-image-20230216151324939.png)
...
![image-20230216151344620](http://img.buxiaoxing.com/uPic/2023/02/16151344-Qmz4wg-image-20230216151344620.png)
...
目前，可以简单的理解为，初始化阶段主要用于产生一个**最终的配置**
...
<img src="http://img.buxiaoxing.com/uPic/2023/02/16151508-Y5D37m-image-20230216151508027.png" alt="image-20230216151508027" style="zoom:50%;" />
...
2. **构建所有依赖模块**
   ...
   ![image-20230216151601132](http://img.buxiaoxing.com/uPic/2023/02/16151601-vqurEq-image-20230216151601132.png)

> AST在线测试工具：https://astexplorer.net/

简图

<img src="http://img.buxiaoxing.com/uPic/2023/02/16151733-8ou0f8-image-20230216151733438.png" alt="image-20230216151733438" style="zoom:50%;" />
...
![image-20230216152115489](http://img.buxiaoxing.com/uPic/2023/02/16152115-mTuz4L-image-20230216152115489.png)
...
![image-20230216152155346](http://img.buxiaoxing.com/uPic/2023/02/16152155-r1kamL-image-20230216152155346.png)

4. **合并chunk assets**

将多个chunk的assets合并到一起，并产生一个总的hash

![image-20230216152230872](http://img.buxiaoxing.com/uPic/2023/02/16152230-54PKEJ-image-20230216152230872.png)

#### 输出

此步骤非常简单，webpack将利用node中的fs模块（文件处理模块），根据编译产生的总的assets，生成相应的文件。

<img src="http://img.buxiaoxing.com/uPic/2023/02/16152256-NcTQFh-image-20230216152256746.png" alt="image-20230216152256746" style="zoom:50%;" />

#### 总过程

![image-20230216152407939](http://img.buxiaoxing.com/uPic/2023/02/16152408-2ZFA3d-image-20230216152407939.png)

![image-20230216152441759](http://img.buxiaoxing.com/uPic/2023/02/16152441-eIngxU-image-20230216152441759.png)
...
![image-20230216152649283](http://img.buxiaoxing.com/uPic/2023/02/16152649-2XkEUa-image-20230216152649283.png)
...
<img src="http://img.buxiaoxing.com/uPic/2023/02/16153455-dgDsLU-image-20230216153455316.png" alt="image-20230216153455316" style="zoom:50%;" />
...
<img src="http://img.buxiaoxing.com/uPic/2023/02/16153733-SoVuII-image-20230216153733493.png" alt="image-20230216153733493" style="zoom:50%;" />
...
这种方式适用于页面之间有一些**独立**、相同的功能，专门使用一个chunk抽离这部分JS有利于浏览器更好的**缓存**这部分内容。

> 思考：为什么不使用多启动模块的方式？(缓存公共模块内容)

#### 单页应用

所谓单页应用，是指整个网站（或网站的某一个功能块）只有一个页面，页面中的内容全部靠JS创建和控制。 vue和react都是实现单页应用的利器。

<img src="http://img.buxiaoxing.com/uPic/2023/02/16153847-5vFJ4j-image-20230216153847205.png" alt="image-20230216153847205" style="zoom:50%;" />
...
webpack loader： loader本质上是一个**函数**，它的作用是将某个**源码字符串转换成另一个源码字符串**返回。

![image-20230216154258563](http://img.buxiaoxing.com/uPic/2023/02/16154258-8viukW-image-20230216154258563.png)

loader函数的将在模块解析的过程中被调用，以得到最终的源码。

**全流程：**

![image-20230216154343207](http://img.buxiaoxing.com/uPic/2023/02/16154343-0vr6av-image-20230216154343207.png)

**chunk中解析模块的流程：**

![image-20230216154404091](http://img.buxiaoxing.com/uPic/2023/02/16154404-kXfGCA-image-20230216154404091.png)

**chunk中解析模块的更详细流程：**

![image-20230216154433193](http://img.buxiaoxing.com/uPic/2023/02/16154433-ip2dbX-image-20230216154433193.png)

**处理loaders流程：**

![image-20230216154508016](http://img.buxiaoxing.com/uPic/2023/02/16154508-fMk5zm-image-20230216154508016.png)
...
function loader(buffer){
  ...
  console.log('图片大小(字节)',size)
	// contenthash：具体某个文件的hash值
  // loaderUtils.getOptions获取webpack.config.js options的配置
  var {limit=1000, filename="[contenthash:5].[ext]"} = loaderUtils.getOptions(this)
  var content = null
  if(size < limit){
 ...
 * 
 * @param {Buffer} buffer 
 * @returns 返回图片的base64格式
 */
function getBase64(buffer){
  return 'data:img/png;base64,'+buffer.toString('base64')
}
...
![image-20230216160135410](http://img.buxiaoxing.com/uPic/2023/02/16160135-Ur5mOs-image-20230216160135410.png)
...
apply函数会在**初始化阶段，创建好Compiler对象后运行**。

compiler对象是在初始化阶段构建的，整个webpack打包期间只有一个compiler对象，后续完成打包工作的是compiler对象内部创建的compilation

apply方法会在**创建好compiler对象后调用**，并向方法传入一个compiler对象

![image-20230216160234602](http://img.buxiaoxing.com/uPic/2023/02/16160234-QW85gt-image-20230216160234602.png)
...
module.exports = class FileList{
  apply(compiler){
    compiler.hooks.emit.tap("FileList", complation => {
      var filelist = []
      console.log(complation.assets)
      for (const key in complation.assets) {
        var content = `【${key}】\n${complation.assets[key].size()/1000} KB`
...
最终打包结果，暴露在全局，例如 `jquery` 就将 `$` 暴露在全局中
...
这比较适用于一些第三方库来自于外部**CDN**的情况，这样一来，即可以在页面中使用CDN，又让bundle的体积变得更小，还不影响源码的编写
...
module.exports = {
    ...
    plugins: [
        ...
        new HtmlWebpackPlugin({
            ...
            filename: "a.html", // 生成的文件名，默认index.html
            chunks: ["a"] // 指定需要引入的chunk，默认为main
        })
    ]
}
```
...
evServer: {
  	port: 8000,
    open: true,
      // 代理规则，通过webpack-dev-server去请求，然后将结果返回给浏览器，因为浏览器端存在跨域问题
     	// webpack-dev-server是node服务器，所以不存在跨域问题
      // 发送http请求如果没有域名会发送到当前服务器(静态资源运行服务器)中去。
      proxy: { 
        "/api": {// 以 /api 开头会发送到target域去.
...
dist
    ...
    |—— scripts
        |—— main.js  #export default "img/a.png"，这里就因为路径问题找不到这个图片
    |—— html
...
output: {
  filename: "scripts/[name].[chunkhash:5].js",
  // 会把这个路径拼接到所有资源路径前面，上面的 img/a.png 就变成了 /img/a.png
  // "/" 表示网站根路径
  // 部署后去请求资源就会请求 http://www.baidu.com/img/a.png
  publicPath: "/" 
},
```
...
所有的webpack内置插件都作为webpack的**静态属性**存在的，使用下面的方式即可创建一个插件对象
...
css-loader的作用，就是将**css代码转换为js代码**
...
<img src="http://img.buxiaoxing.com/uPic/2023/02/16234050-0Dsn9b-image-20230216234050005.png" alt="image-20230216234050005" style="zoom:50%;" />
...
css in js 的核心思想是：用一个**JS对象**来描述样式，而不是css样式表
...
- **在页面中增加了大量冗余内容**：在页面中处理css in js时，往往是将样式**加入到元素的style属性**中，会大量增加元素的内联样式，并且可能会有大量重复，不易阅读最终的页面代码
...
![image-20230216234612655](http://img.buxiaoxing.com/uPic/2023/02/16234612-42LkMg-image-20230216234612655.png)
...
![image-20230216234639017](http://img.buxiaoxing.com/uPic/2023/02/16234639-SPGbGY-image-20230216234639017.png)

原理极其简单，开启了css module后，css-loader会将样式中的类名进行转换，转换为一个唯一的hash值。

由于hash值是根据**模块路径和类名**生成的，因此，不同的css模块，哪怕具有相同的类名，转换后的hash值也不一样。

![image-20230216234724303](http://img.buxiaoxing.com/uPic/2023/02/16234724-xlqwlg-image-20230216234724303.png)

#### 如何应用样式

css module带来了一个新的问题：源代码的类名和最终生成的类名是不一样的，而开发者只知道自己写的源代码中的类名，并不知道最终的类名是什么，那如何应用类名到元素上呢？

为了解决这个问题，css-loader会导出原类名和最终类名的**对应关系**，该关系是通过一个对象描述的

![image-20230216234811174](http://img.buxiaoxing.com/uPic/2023/02/16234811-lTSdF7-image-20230216234811174.png)
...
![image-20230216235154935](http://img.buxiaoxing.com/uPic/2023/02/16235155-16ceNh-image-20230216235154935.png)

目前，最流行的预编译器有**LESS**和**SASS**，由于它们两者特别相似，因此仅学习一种即可（本课程学习LESS）

![image-20230216235224438](http://img.buxiaoxing.com/uPic/2023/02/16235224-dfV0uu-image-20230216235224438.png)
...
module.exports = {
    ...
    module: {
        rules: [
          ...
          // 通过less-loader 将 less->css 再交给css-loader处理
...
![image-20230216235942176](http://img.buxiaoxing.com/uPic/2023/02/16235942-Fr5syx-image-20230216235942176.png)

看上去是不是和LESS、SASS一样呢？

但PostCss和LESS、SASS的思路不同，它其实只做一些代码分析之类的事情，将**分析的结果交给插件**，具体的代码转换操作是插件去完成的。

![image-20230217000026132](http://img.buxiaoxing.com/uPic/2023/02/17000026-UxLKPZ-image-20230217000026132.png)

官方的一张图更能说明postcss的处理流程：

<img src="http://img.buxiaoxing.com/uPic/2023/02/17000051-8qjlLE-image-20230217000050904.png" alt="image-20230217000050904" style="zoom:50%;" />
...
postcss库提供了对应的js **api**用于转换代码，如果你想使用postcss的一些高级功能，或者想开发postcss插件，就要api使用postcss，api的文档地址是：http://api.postcss.org/
...
![image-20230217000803772](http://img.buxiaoxing.com/uPic/2023/02/17000803-hWU144-image-20230217000803772.png)
...
![image-20230217135713108](http://img.buxiaoxing.com/uPic/2023/02/17135713-NVpnNo-image-20230217135713108.png)

由于语言的转换工作灵活多样，babel的做法和postcss、webpack差不多，它本身仅提供一些分析功能，真正的转换需要依托于插件完成

![image-20230217135736251](http://img.buxiaoxing.com/uPic/2023/02/17135736-3299dv-image-20230217135736251.png)
...
function _instanceof(left, right) {
    if (right != null && typeof Symbol !== "undefined" && right[Symbol.hasInstance]) 		{
      	// ES6中判断每个类中都有一个方法 right[Symbol.hasInstance](left) 判断left是不是通过					// right构建的实例
...
<img src="http://img.buxiaoxing.com/uPic/2023/02/17155959-nhH0f8-image-20230217155959492.png" alt="image-20230217155959492" style="zoom:50%" />
...
3. 浏览器缓存：JS文件会被浏览器**缓存**，被缓存的文件不会再进行传输
...
![image-20230217160243512](http://img.buxiaoxing.com/uPic/2023/02/17160243-8kdYxB-image-20230217160243512.png)

模块解析包括：抽象语法树分析、依赖分析、模块语法替换

#### 不做模块解析会怎样？

![image-20230217160301046](http://img.buxiaoxing.com/uPic/2023/02/17160301-mWD9l6-image-20230217160301046.png)

如果某个模块不做解析，该模块经过loader处理后的代码就是最终代码。

如果没有loader对该模块进行处理，该**模块的源码就是最终打包结果**的代码。
...
![image-20230217160819717](http://img.buxiaoxing.com/uPic/2023/02/17160819-kZbwR3-image-20230217160819717.png)
...
![image-20230217161237262](http://img.buxiaoxing.com/uPic/2023/02/17161237-09PY4d-image-20230217161237262.png)

而使用了热替换后，流程发生了变化

![image-20230217161254240](http://img.buxiaoxing.com/uPic/2023/02/17161254-7qADCE-image-20230217161254240.png)
...
![image-20230217161356743](http://img.buxiaoxing.com/uPic/2023/02/17161356-7bq9el-image-20230217161356743.png)
...
![image-20230217161556929](http://img.buxiaoxing.com/uPic/2023/02/17161557-mH94iC-image-20230217161556929.png)
...
![image-20230217162306514](http://img.buxiaoxing.com/uPic/2023/02/17162306-seQMSJ-image-20230217162306514.png)
...
gzip是一种压缩文件的算法：对重复字符串优化

#### B/S结构中的压缩传输

![image-20230218094627190](http://img.buxiaoxing.com/uPic/2023/02/18094627-HGZtxt-image-20230218094627190.png)
...
![image-20230218094654808](http://img.buxiaoxing.com/uPic/2023/02/18094655-pkcRXH-image-20230218094654808.png)
...
if (Math.random() < 0.5) {
  	// 会将 ./utils/ 下的所有模块导入打包
  	// webpack会使用 require.context()
   const a = require("./utils/" + module); 
 		// 如果连大概位置都没有，webpack则不会导入任何模块
...
const context = require.context("./utils", true, /\.js$/); // 返回一个函数
const a = context("./a.js") // context里面记录了键值对 相当于导入./utils/a.js 

console.log(context.keys()) // 键的数组
```
